...
import (
	...
	"github.com/confero-network/go-confero/common/prque"
	...
	"github.com/confero-network/go-confero/internal/syncx"
...
// available in the database. It initialises the default Confero Validator
// and Processor.
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Gcofe is initialized with
...
// it's not checked in the Gcofe side anymore.
...
// sync, Gcofe will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Gcofe with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Gcofe to a new version which supports transaction
