...
// the process. This is usually the case when Gcofe is shutting down and some events
...
//
// Opposed to the original Confero block synchronization which is trustless (and
...
// not from within (vs. legacy Confero sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gcofe is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gcofe requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
