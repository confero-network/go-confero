...
var (
	...
	// testAddr is the Confero address of the tester account.
...
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *eth.Confero) {
	t.Helper()
...
func setupBlocks(t *testing.T, ethservice *eth.Confero, n int, parent *types.Block, callback func(parent *types.Block)) {
	api := NewConsensusAPI(ethservice)
	for i := 0; i < n; i++ {
		callback(parent)
