...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gcofe is "gcofe". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gcofe", keep that.
	if name == "gcofe" || name == "gcofe-testnet" {
		name = "Gcofe"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gcofe" instances.
var isOldGcofeResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gcofe 1.4 are used if they exist.
	if warn, isOld := isOldGcofeResource[path]; isOld {
		oldpath := ""
		if c.name() == "gcofe" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGcofeResourceWarning, "Using deprecated resource file %s, please move this file to the 'gcofe' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-confero-keystore")
		isEphemeral = true
	}
