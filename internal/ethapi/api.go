...
import (
	...
	"github.com/confero-network/go-confero/accounts/keystore"
	"github.com/confero-network/go-confero/accounts/scwallet"
	"github.com/confero-network/go-confero/common"
	"github.com/confero-network/go-confero/common/hexutil"
	"github.com/confero-network/go-confero/common/math"
	"github.com/confero-network/go-confero/consensus/ethash"
	"github.com/confero-network/go-confero/consensus/misc"
	"github.com/confero-network/go-confero/core"
	"github.com/confero-network/go-confero/core/state"
	...
	"github.com/confero-network/go-confero/eth/tracers/logger"
...
// ConferoAPI provides an API to access Confero related information.
type ConferoAPI struct {
	b Backend
}

// NewConferoAPI creates a new Confero protocol API.
func NewConferoAPI(b Backend) *ConferoAPI {
	return &ConferoAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *ConferoAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *ConferoAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *ConferoAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *ConferoAPI) Syncing() (interface{}, error) {
...
// ConferoAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type ConferoAccountAPI struct {
	am *accounts.Manager
}

// NewConferoAccountAPI creates a new ConferoAccountAPI.
func NewConferoAccountAPI(am *accounts.Manager) *ConferoAccountAPI {
	return &ConferoAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *ConferoAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Confero ECDSA signature for:
// keccak256("\x19Confero Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/confero-network/go-confero/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Confero Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/confero-network/go-confero/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Confero signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gcofe.
...
// BlockChainAPI provides an API to access Confero blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Confero blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Confero chain config.
//
...
// See: https://github.com/confero-network/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/confero-network/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Confero APIs exposed over the debugging
// namespace.
...
// Version returns the current confero protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Cofe)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f cofe) exceeds the configured cap (%.2f cofe)", feeFloat, cap)
	}
	return nil
}
