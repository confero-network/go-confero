...
import (
	...
	"github.com/confero-network/go-confero/core/beacon"
...
type ethNode struct {
	...
	ethBackend *eth.Confero
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightConfero
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.Confero, *ethcatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Confero node
	...
	// Create the node and configure a full Confero node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightConfero, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Confero node
	datadir, _ := os.MkdirTemp("", "")
