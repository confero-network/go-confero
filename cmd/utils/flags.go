...
// Package utils contains internal helper functions for go-confero commands.
package utils
...
import (
	...
	"github.com/confero-network/go-confero/common/fdlimit"
	...
	"github.com/confero-network/go-confero/eth"
	ethcatalyst "github.com/confero-network/go-confero/eth/catalyst"
	"github.com/confero-network/go-confero/eth/downloader"
	"github.com/confero-network/go-confero/eth/ethconfig"
	"github.com/confero-network/go-confero/eth/filters"
	"github.com/confero-network/go-confero/eth/gasprice"
	"github.com/confero-network/go-confero/eth/tracers"
	"github.com/confero-network/go-confero/ethdb"
	"github.com/confero-network/go-confero/ethdb/remotedb"
	...
	"github.com/confero-network/go-confero/internal/flags"
	"github.com/confero-network/go-confero/les"
	lescatalyst "github.com/confero-network/go-confero/les/catalyst"
	"github.com/confero-network/go-confero/log"
	"github.com/confero-network/go-confero/metrics"
	"github.com/confero-network/go-confero/metrics/exp"
	"github.com/confero-network/go-confero/metrics/influxdb"
	...
	"github.com/confero-network/go-confero/p2p/netutil"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in cofe) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Gcofe will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Gcofe configurations storing the
...
// for Gcofe and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gcofe account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Confero client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Confero) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Confero service: %v", err)
		}
...
// RegisterEthStatsService configures the Confero Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Confero Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gcofe.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gcofe.", tagsMap)
		}
