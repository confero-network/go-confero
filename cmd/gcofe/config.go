...
import (
	...
	"github.com/confero-network/go-confero/accounts/external"
	"github.com/confero-network/go-confero/accounts/keystore"
	"github.com/confero-network/go-confero/accounts/scwallet"
	"github.com/confero-network/go-confero/accounts/usbwallet"
...
func loadConfig(file string, cfg *gcofeConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gcofe.ipc"
	return cfg
}

// makeConfigNode loads gcofe configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gcofeConfig) {
	// Load defaults.
	cfg := gcofeConfig{
...
// makeFullNode loads gcofe configuration and creates the Confero backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gcofe db freezer-migrate`.")
		}
	}
	...
	// Add the Confero Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gcofeConfig) {
