...
`
	...
	{
		gcofe := runGcofe(t, "account", "list", "--datadir", datadir)
		gcofe.Expect(want)
		gcofe.ExpectExit()
	}
	{
		gcofe := runGcofe(t, "--datadir", datadir, "account", "list")
		gcofe.Expect(want)
		gcofe.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gcofe := runGcofe(t, "account", "new", "--lightkdf")
	defer gcofe.ExpectExit()
	gcofe.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gcofe := runGcofe(t, "account", "-h")
	gcofe.WaitExit()
	if have, want := gcofe.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gcofe = runGcofe(t, "account", "import", "-h")
	gcofe.WaitExit()
	if have, want := gcofe.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gcofe := runGcofe(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gcofe.ExpectExit()
	gcofe.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gcofe := runGcofe(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gcofe := runGcofe(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gcofe.ExpectExit()
	gcofe.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gcofe.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gcofe := runMinimalGcofe(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gcofe.StderrText(), m) {
...
// https://github.com/confero-network/go-confero/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gcofe.SetTemplateFunc("keypath", func(file string) string {
