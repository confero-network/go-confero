// Copyright 2020 The go-confero Authors
...
func (g *gcoferpc) killAndWait() {
	g.gcofe.Kill()
	g.gcofe.WaitExit()
}

func (g *gcoferpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gcofe.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gcoferpc) addPeer(peer *gcoferpc) {
	g.gcofe.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gcofe.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gcofe.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gcofe.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gcofe.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gcoferpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startGcofeWithIpc(t *testing.T, name string, args ...string) *gcoferpc {
	ipcName := fmt.Sprintf("gcofe-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	args = append([]string{"--networkid=42", "--port=0", "--authrpc.port", "0", "--ipcpath", ipcName}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)

	g := &gcoferpc{
		name: name,
		gcofe: runGcofe(t, args...),
	}
	ipcpath := ipcEndpoint(ipcName, g.gcofe.Datadir)
	// We can't know exactly how long gcofe will take to start, so we try 10
...
func initGcofe(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gcofe: %v ", args)
	g := runGcofe(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gcoferpc {
	datadir := initGcofe(t)
	t.Logf("Importing keys to gcofe")
	runGcofe(t, "account", "import", "--datadir", datadir, "--password", "./testdata/password.txt", "--lightkdf", "./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGcofeWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gcoferpc {
	datadir := initGcofe(t)
	return startGcofeWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
