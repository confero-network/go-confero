...
import (
	...
	"github.com/confero-network/go-confero/les/utils"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // Confero protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local confero node is running in archive mode, advertise ourselves we have
...
// participating in the Light Confero sub-protocol.
