...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Confero address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Confero account at a particular block.
    type Account {
    ...
    # Transaction is an Confero transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Confero account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Confero account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Confero block by number or by hash. If neither is
