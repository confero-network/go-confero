...
// Package accounts implements high level Confero account management.
package accounts
...
// Account represents an Confero account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Confero account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain confero.ChainStateReader)
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Confero Signed Message:\n%d%s", len(data), string(data))
