...
import (
	...
	"github.com/confero-network/go-confero/accounts/usbwallet/trezor"
...
// or whether the Confero app was not started on it.
...
// Confero address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.ConferoAddress)
	if _, err := w.trezorExchange(&trezor.ConferoGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.ConferoTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.ConferoTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
