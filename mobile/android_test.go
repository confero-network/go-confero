...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gcofe.LightScryptN, Gcofe.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.confero", new NodeConfig());
			node.start();
			...
			ConferoClient ec = node.getConferoClient();
	...
	// signatures too. Regression test for go-confero issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/confero/gcofetest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gcofe and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.confero", "github.com/confero-network/go-confero/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gcofe.aar"), "gcofe.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "confero", "gcofetest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
