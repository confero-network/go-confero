...
// Package gcofeclient provides an RPC client for gcofe-specific APIs.
package gcofeclient
...
// Client is a wrapper around rpc.Client that implements gcofe-specific functionality.
//
// If you want to use the standardized Confero RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg confero.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg confero.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gcofe node.
...
// MemStats retrieves the current memory stats from a gcofe node.
...
// GetNodeInfo retrieves the node info of a gcofe node.
