...
// Verify that Client implements the confero interfaces.
var (
	_ = confero.ChainReader(&Client{})
	_ = confero.TransactionReader(&Client{})
	_ = confero.ChainStateReader(&Client{})
	_ = confero.ChainSyncReader(&Client{})
	_ = confero.ContractCaller(&Client{})
	_ = confero.GasEstimator(&Client{})
	_ = confero.GasPricer(&Client{})
	_ = confero.LogFilterer(&Client{})
	_ = confero.PendingStateReader(&Client{})
	// _ = confero.PendingStateEventer(&Client{})
	_ = confero.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Confero Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new confero service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == confero.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != confero.NotFound {
		t.Fatal("error should be confero.NotFound")
	}
}
